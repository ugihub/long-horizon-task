...
Expected: All assertions pass, tracker printed, events listed, final "[x] Tahap 1 E2E passed!"
...
- [x] P0: state schema, task model, 8 statuses, 12 phases, transition tables, event log, plan/update schema, security policy, modes -- all covered in Tasks 1-4
- [x] P1: `.lhtm/` directory structure (Task 3), state_store atomic (Task 3), schema_validator (Task 4), goal hash (Task 2), markdown_view (Task 6), planner skill (Task 7), planning flow (Task 8 engine) -- all covered
- [x] P2: lhtm_core, planner, executor, verifier, recovery, output_contract (Task 7), parser + repair (Task 5), task card injection format (example), examples, policies -- all covered
- [x] No placeholders -- every step has complete code, expected commands, and test output
- [x] Type consistency -- all functions/methods used in later tasks (engine.py) match definitions in earlier tasks
- [x] TDD -- every task writes tests first, runs to see fail, implements, runs to see pass