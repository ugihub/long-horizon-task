...
**Constraint: ALL output ASCII.** No non-ASCII glyphs (no `→`, `✓`, `–`) in code, comments, or commit messages. Windows cp1252.
...
- **P6 (spec §4.1):** recovery.py (Task 2) + engine `recover`/`set_mode` (Task 3). All six actions implemented; targets map to legal transitions; `decompose_task` validates subtasks via `validate_plan` and adds the parent edge on apply; `switch_to_safe_mode` rejects FULL_AUTO; corrupt-state recovery reuses existing `store.restore_snapshot` (no new code, per spec).
- **P7 redactor (§4.2):** redactor.py (Task 1) + hooks in context_builder (Task 5), safe_executor + markdown_view (Task 7). Model-facing only; state/verifier stay raw; `redact_secrets` flag respected in context_builder + executor; markdown render redaction is opt-in (redactor must be attached).
- **P7 runbook (§4.3):** runbook.py (Task 4). Declarative schema, validate/execute, run_command/write_file/assert, idempotency by step id, `.bak` backup (executor), per-step timeout (executor `max_cmd_timeout`), dry-run, stop-on-failure. Operator-authored only.
- **P7 context_budget (§4.4):** context_budget.py (Task 5). Hierarchy ratios, per-section caps, truncate suffix, cascade drop, never exceeds budget. `context_builder.py` becomes a thin wrapper + redaction hook; existing 5 builder tests still pass (content preserved).
- **P7 project_facts (§4.5):** project_facts.py (Task 6) + engine `refresh_facts`. Scans allowed_paths, excludes blocked, summary capped, excerpts for context budget.
- **Mode escalation (§4.6):** action_gate AUTO_SAFE/DRY_RUN/FULL_AUTO tiering (Task 8). AUTO_SAFE auto-approves only low-risk (and overwrite still forced by `require_for_file_overwrite`); DRY_RUN marks decisions and executor no-ops; `set_mode` validates (Task 3).
