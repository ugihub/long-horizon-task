...
- **P3 coverage (spec sec.1, sec.3):** task_scheduler (T2), context_builder (T6), action_gate (T4), safe_executor (T5), prompt_loader (T3), audit (T6), config (T1). All 7 spec components mapped.
- **Approval callback (sec.4.2):** gate returns `requires_approval`; driver (demo/test) grants via `approval_granted`. T4 test + T8 demo prove it.
- **Security rules (sec.5):** path allowed (T4 test_write_outside_allowed_path_rejected), sensitive blocklist (T4 test_read_sensitive_blocked), allowlist command (T4 test_run_command_not_in_allowlist), destructive (T4 test_run_command_destructive_denied), active-task binding (T4 test_active_task_mismatch_rejected). All covered.
- **Supervised default (sec.4.7):** T1 default mode supervised; T7 ships `.lhtm/config.yaml`.
- **Contract extension (sec.4.8):** T7 updates output_contract.md; parser/validator unchanged (proposed_actions is optional passthrough).
- **Deferred (Non-Goals):** no evidence_verifier, no recovery, no redactor, no runbook, no real-LLM binding -- consistent with spec sec.2.
