...
- **P4 coverage (spec sec.3, sec.4):** evidence_verifier.py (Task 1), engine hook on claimed_done (Task 2), deterministic-only (no LLM pass). All spec checks C1-C5 implemented.
- **Fail path (sec.4.2):** fail -> `failed` + `task["feedback"]`, `active_task_id` cleared -- Task 2. Reuses existing `failed` status (decision), no 9th status.
- **P5 gaps (sec.4.3):** configurable `max_cmd_timeout` (Task 3); `max_repair_attempts` already in config and enforced by existing `attempts >= max_attempts` gate (documented, no code). Output summarization + destructive blocklist already exist (Tahap 2).
- **Verified is engine-owned:** schema_validator already rejects LLM `verified_done`; engine is the only writer (Task 2) -- matches spec sec.5.
- **Deferred (Non-Goals):** no LLM verifier second pass, no `needs_more_work` status, no recovery/redactor/runbook/context_budget -- consistent with spec sec.2.
