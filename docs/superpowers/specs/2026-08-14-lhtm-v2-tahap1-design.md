...
### 3.4 Fase (Phase 4.4)
12 fase (sumber: `Implementation_plan.md` sec.4.4): `DRAFT, PLANNING, PLAN_REVIEW, READY, EXECUTING, VERIFYING, BLOCKED, WAITING_USER, FAILED, RECOVERY, COMPLETED, ABORTED`.

Model transisi = **state machine dengan edge eksplisit**, bukan monotonic index (dari contoh legal di sec.4.4: `VERIFYING->READY`, `RECOVERY->READY` legal walau mundur). Tabel edge `PHASE_TRANSITIONS`:
